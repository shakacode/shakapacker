describe "Command" do
  ...
  describe "#compile" do
    it "returns success status when stale" do
      allow(Webpacker.compiler).to receive(:stale?).and_return(true)
      allow(Webpacker.compiler).to receive(:run_webpack).and_return(true)

      expect(Webpacker.commands.compile).to be true
    end

    it "returns success status when fresh" do
      allow(Webpacker.compiler).to receive(:stale?).and_return(false)
      allow(Webpacker.compiler).to receive(:run_webpack).and_return(true)
    ...
    it "returns failure status when stale" do
      allow(Webpacker.compiler).to receive(:stale?).and_return(true)
      allow(Webpacker.compiler).to receive(:run_webpack).and_return(false)
  ...
  describe "#clean" do
    before :context do
      ...
      @file_delete_mock = double("File Delete")

      @file_mtime_stub = Proc.new { |longpath| @all_files[longpath] }
    ...
    private
      ...
      def with_time_and_files_stub(&proc)
