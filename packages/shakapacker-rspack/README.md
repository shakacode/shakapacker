...
```sh
yarn add --dev 'shakapacker@~10.1.0' 'shakapacker-rspack@~10.1.0' '@rspack/core@~2.0.1' '@rspack/cli@~2.0.1' 'rspack-manifest-plugin@~5.2.1'
```

`@rspack/cli` is a required peer because `bin/shakapacker` (Shakapacker's standard build/dev-server entrypoint) shells out to the `rspack` CLI binary rather than driving Rspack via the JS API.
