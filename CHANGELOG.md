...
- **Fixed `require("shakapacker")` and `require("shakapacker/rspack")` triggering plugin initialization as a side effect**. [PR #1107](https://github.com/shakacode/shakapacker/pull/1107) by [justin808](https://github.com/justin808). `baseConfig` is now exposed via a lazy getter, and the rspack `rules` export is also lazy, so `ensureManifestExists`, plugin constructors, and (for rspack) `@rspack/core` / `rspack-manifest-plugin` resolution are delayed until config generation or the relevant lazy export is actually requested. Fixes [#1095](https://github.com/shakacode/shakapacker/issues/1095). Note: as a side-effect, `shakapacker.baseConfig` is now effectively read-only — direct reassignment (`shakapacker.baseConfig = ...`) silently fails in sloppy mode and throws in strict mode. Use `Object.defineProperty` to override if you previously relied on reassignment.
