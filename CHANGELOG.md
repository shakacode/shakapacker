...
- Topological ordering of a directed graph is a linear ordering mechanism to preserve the order of vertices for all edges in graph. Added `use_topological_order` flag to configure `javascript_pack_tag` and `stylesheet_pack_tag` to return packs in Topological Order by [pulkitkkr](https://github.com/shakacode/shakapacker/pull/185) in [PR 193](https://github.com/shakacode/shakapacker/pull/193). 
