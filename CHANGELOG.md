...
- **Breaking: some native ESM named imports from `shakapacker` and `shakapacker/rspack` no longer work**. [PR #1107](https://github.com/shakacode/shakapacker/pull/1107) by [justin808](https://github.com/justin808). The side-effect fix for [#1095](https://github.com/shakacode/shakapacker/issues/1095) reimplements `baseConfig` and `rules` as lazy getters, so they are no longer statically detected as CommonJS named exports. For `shakapacker/rspack`, native ESM consumers must access `baseConfig`/`rules` via the default import (`import shakapacker from 'shakapacker/rspack'; const { baseConfig, rules } = shakapacker`) rather than `import { baseConfig, rules } from 'shakapacker/rspack'`, which now throws a `SyntaxError` at load time. For the webpack entry, `shakapacker` is now assembled into a local object before its CommonJS `export =`, which removes it from native ESM's static named-export detection entirely: `import { config } from 'shakapacker'` (the one member Node previously resolved) now throws a `SyntaxError` — use the default import (`import shakapacker from 'shakapacker'`) instead. In all cases `generateWebpackConfig`/`generateRspackConfig` and the other exports are unaffected, and CommonJS access (`require('shakapacker').baseConfig`, `require('shakapacker/rspack').baseConfig`) — including direct assignment to override the lazy value — continues to work.
