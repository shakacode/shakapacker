...
- **Added AI analysis prompt generation to the config exporter**. [PR #695](https://github.com/shakacode/shakapacker/pull/695) by [justin808](https://github.com/justin808). When running doctor mode (`bin/shakapacker-config --doctor`), the exporter now also writes an `AI-ANALYSIS-PROMPT.md`: a ready-to-paste prompt that guides an AI assistant (ChatGPT, Claude, Gemini, etc.) to review the configuration for migration issues (webpack ↔ rspack), build errors, client/server and development/production optimizations, and best practices. This is purely additive; existing exports are unchanged, and a failure to write the prompt file only warns without affecting the exported configs.
...
- **Breaking: `shakapacker.baseConfig` and `shakapacker.rules` (plus the matching `shakapacker/rspack` exports) are now read-only**. [PR #1107](https://github.com/shakacode/shakapacker/pull/1107) by [justin808](https://github.com/justin808). The side-effect fix for [#1095](https://github.com/shakacode/shakapacker/issues/1095) reimplements these exports as lazy getters, so direct reassignment (`shakapacker.baseConfig = customConfig`, `shakapacker.rules = customRules`, etc.) now throws an informative `TypeError`. Consumers that previously mutated these exports must switch to `Object.defineProperty(shakapacker, 'baseConfig', { value: customConfig })` (or the matching `rules` form) — the thrown error includes this guidance inline. Additionally, because the `shakapacker/rspack` `baseConfig` and `rules` exports are now installed as lazy getters, they are no longer statically detected as named exports; native ESM consumers must access them via the default import (`import shakapacker from 'shakapacker/rspack'; const { baseConfig, rules } = shakapacker`) rather than `import { baseConfig, rules } from 'shakapacker/rspack'`, which now throws a `SyntaxError` at load time. `generateRspackConfig` and the other named exports are unaffected.
