...
- **Added AI analysis prompt generation to the config exporter**. [PR #695](https://github.com/shakacode/shakapacker/pull/695) by [justin808](https://github.com/justin808). When exporting configs (for example, `bin/shakapacker-config --doctor`), the exporter now also writes an `AI-ANALYSIS-PROMPT.md`: a ready-to-paste prompt that guides an AI assistant (ChatGPT, Claude, Gemini, etc.) to review the configuration for migration issues (webpack ↔ rspack), build errors, client/server and development/production optimizations, and best practices. This is purely additive; existing exports are unchanged.
