...
- **Added supplemental npm packages `shakapacker-webpack` and `shakapacker-rspack`**. [PR #1096](https://github.com/shakacode/shakapacker/pull/1096) by [justin808](https://github.com/justin808). Optional packages that lockstep with core and pin the bundler/transpiler peer set against the versions Shakapacker is tested with, so installers see one consistent peer surface instead of resolving conditional peers through core. Both required and optional peers use patch-tolerant `~X.Y.Z` ranges so routine semver-patch bumps in the consumer's app do not trigger npm 7+ peer warnings until a Shakapacker release. Both packages emit `process.emitWarning` with code `SHAKAPACKER_BUNDLER_MISMATCH` when `config.assets_bundler` does not match the wrapper (`shakapacker-webpack` warns when it is set to a non-webpack value; `shakapacker-rspack` warns when it is anything other than `rspack`, including unset, since rspack must be opted into). `shakapacker-webpack` additionally emits `SHAKAPACKER_NO_TRANSPILER` when the configured `javascript_transpiler` pair is not installed. See `docs/rfcs/0001-tighten-dependencies-v11.md` for the phased rollout plan; v11 will remove these peers from core entirely.
