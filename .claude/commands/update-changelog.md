...
2. **Do NOT add entries for**:
   ...
   - CI/CD changes
   - **RC-only regression fixes**: a fix for a bug that was introduced by another PR which itself only shipped in an unreleased prerelease (e.g., `v10.1.0-rc.0`). Users upgrading from the last stable version to the next prerelease/stable never see the bug, so the fix is internal RC churn. See "Filtering RC-only fixes when collapsing prereleases" below.
...
5. **Filtering RC-only fixes when collapsing prereleases**: while collapsing, scan the post-rc commit list for fixes that exist only to repair a regression introduced earlier in the same prerelease train, and **drop those entries** from the collapsed section:
   ...
   - If the referenced PR is **already listed** in the prerelease section being collapsed (i.e., it shipped only in an RC, not in the last stable version), the fix is RC-only churn — remove it from the changelog and surface this decision in the summary
   - If the referenced PR shipped in the last stable version (or earlier), keep the fix — real users running the stable release will see the bug
   - When uncertain, ask the user before dropping
   - Example: rc.0 ships PR #1110 (gem allowlist). Post-rc.0, PR #1120 fixes a regression PR #1110 caused. When stamping rc.1, drop PR #1120 — `v10.0.0 → v10.1.0-rc.1` consumers never see the bug
   - Always **list dropped RC-only fixes in the final summary** so the user can override if they want to keep one for RC testers
