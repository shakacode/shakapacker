...
### Topological Ordering of Packs

A topological ordering of a directed graph is a linear ordering mechanism to preserve the order of vertices for all edges in the graph. Set the `use_topological_order: true` flag to configure `javascript_pack_tag` and `stylesheet_pack_tag` to return packs in topological order.

Topological order ensures script and JS tags order all dependencies based on the order of all included entry points from the manifest. The default method appends dependencies by order of included pack tags. 
