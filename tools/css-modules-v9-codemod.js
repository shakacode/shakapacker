...
module.exports = function transformer(fileInfo, api) {
  ...
  root.find(j.ImportDeclaration, {
    source: {
      value: value => value && value.match(/\.module\.(css|scss|sass|less)$/)
    }
  }).forEach(path => {
    ...
    if (isTypeScript) {
      ...
      console.log(`✅ [TS] Updated ${fileInfo.path}: import * as ${defaultImportName} from '${sourcePath}'`);
    } else {
      ...
      if (usages.size > 0) {
        ...
        const namedSpecifiers = Array.from(usages).sort().map(name => {
          ...
          if (camelCaseName !== name) {
            ...
            console.log(`  ⚠️  Note: '${name}' will be imported as '${camelCaseName}' due to camelCase conversion`);
        ...
        hasChanges = true;
        console.log(`✅ [JS] Updated ${fileInfo.path}: import { ${Array.from(usages).join(', ')} } from '${sourcePath}'`);
        ...
        console.log(`⚠️  [JS] No specific class usages found in ${fileInfo.path}, converting to namespace import`);
